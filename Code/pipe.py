...
mugenx='"C:/Users/Jadon/Work Space 4Y/Final Research Project/Generating-AI-Using-GAs/MUGEN/mugen-1.0/mugen/mugen.exe"' 
logx = '"C:/Users/Jadon/Work Space 4Y/Final Research Project/Generating-AI-Using-GAs/Code/Logs/G{0}_{1}.txt"'
mugendr = 'C:/Users/Jadon/Work Space 4Y/Final Research Project/Generating-AI-Using-GAs/MUGEN/mugen-1.0/mugen/'
...
def update_stats_sheet(winner, looser, i):
    df = list_df_stats[i]
    ...
    if winsp[0] == losp[0]:
        print("Draw between {0} and {1}".format(winsp[1], losp[1]))
        #Update draws
        #df.loc[df.loc[df.Name == winner].index,'Draws'] +=1
        
        
    elif 'dArwIn_' in winner:
        #"Gen":[gen], "Name":names,"Wins":[de],"Loses":[de], "Draws"
        #Update wins losses
        df.loc[df.loc[df.Name == winner].index,'Wins'] +=1
        
              
    elif 'dArwIn_' in looser:
        df.loc[df.loc[df.Name == looser].index, 'Loses'] +=1
...
def build_cmds(round_list, gen):
    mugen_exe = mugenx#'"C:/Users/Jadon/Work Space 4Y/RP/MUGEN/mugen-1.0/mugen/mugen.exe"'
    num_rounds = 1
    logs=[]
    cmds = []
    for i in np.arange(len(round_list)):
        log_i = logx.format(gen,i)  #'"C:/Users/Jadon/Work Space 4Y/RP/Code/Logs/G{0}_{1}.txt"'.format(gen,i)
...
def build_cmds_to_assess(round_list, gen):
    mugen_exe = mugenx#'"C:/Users/Jadon/Work Space 4Y/RP/MUGEN/mugen-1.0/mugen/mugen.exe"'
    num_rounds = 1
    logs=[]
    cmds = []
    ...
    for i in np.arange(len(round_list)):
        ...
        for j in np.arange(len(round_list[i])):
            log_i = logx.format(str(gen),str(match_no)) #'"C:/Users/Jadon/Work Space 4Y/RP/Code/Logs/Ass_G{0}_{1}.txt"'.format(str(gen),str(match_no))
            x = "{0} -log {1} -p1 {2} -p2 {3} -p1.ai 1 -p2.ai 1 -rounds {4} -nojoy -nosound".format(mugen_exe,log_i, round_list[i][j][0], round_list[i][j][1], num_rounds)
            cmds.append(x)
            logs.append(log_i)
            match_no+=1
    return cmds, logs
...
def assess_round_robin_parallel(commands, logs, generation):
    
    mugen_dir = mugendr#'C:/Users/Jadon/Work Space 4Y/RP/MUGEN/mugen-1.0/mugen/'
    ...
    for i in np.arange(len(winlist)):
        ...
        update_stats_sheet(winlist[i],loslist[i],generation)
        
        print("Winner : "+winlist[i])
...
def nu_gen(df,size,nextgen):
    ...
    new_pop = []
    df_newPop = pd.DataFrame(columns=['Name','ELO','Move_Seq','Wins','Losses','Total_Matches'])
    ...
    for i in np.arange(loop):
        ...
        elo = 1000
        matches = 0
        #move sequences for each child is the same as their base parent
        move_sq_a = df.loc[df.Name==par1].Move_Seq.values.tolist()[0]
        move_sq_b = df.loc[df.Name==par2].Move_Seq.values.tolist()[0]
        dfa = pd.DataFrame({'Name':[aname], 'ELO':[elo], 'Move_Seq':[move_sq_a] ,'Wins':[winlos], 'Losses':[winlos], 'Total_Matches':[matches]}, columns=['Name','ELO','Move_Seq','Wins','Losses', 'Total_Matches'])
        dfb = pd.DataFrame({'Name':[bname], 'ELO':[elo], 'Move_Seq':[move_sq_b] ,'Wins':[winlos], 'Losses':[winlos], 'Total_Matches':[matches]}, columns=['Name','ELO','Move_Seq','Wins','Losses', 'Total_Matches'])
    ...
    if num_nu==0:
        ...
        for ind in np.arange(num_nu):
             ...
             sq_lis=alpha_chargen.spawn_randomly(name_nu)
             df_nu = pd.DataFrame({'Name':[name_nu], 'ELO':[elo_nu], 'Move_Seq':[sq_lis],'Wins':[winl], 'Losses':[winl],'Total_Matches':[matches]}, columns=['Name','ELO','Move_Seq','Wins','Losses','Total_Matches'])
    ...
    print("New children : {0} ".format(tloop))
...
def init_assess_df(names, gen):
    de =np.zeros(len(names), dtype=np.int)
    gens=np.empty(len(names), dtype=np.int)
    gens.fill(gen)
...
def evolve_exp_std(num_generations, current_pop):
    for i in np.arange(num_generations):
        ...
        if i < num_generations-1:        
            ...
            #Now we can assess the children at the next generation
            
            #Initilize the stats df list for these children
            init_assess_df(children,i+1)
            #get lineup
            child_line_up = assess_names(children, i+1)
            print(child_line_up)
            #build up cmds for children line up
            cmds, logs = build_cmds_to_assess(child_line_up, i+1)
            #df_stats = pd.DataFrame(columns=["Gen","Name","Wins","Loses","Draws"])
            assess_round_robin_parallel(cmds, logs, i+1)
...
#Initial Population
init_pop_names, move_seqs, elos = alpha_chargen.initial_population(num_init_pop, 0)
#init_pop = ['dArwIn_G0_0']

#elo = [1000]

df_init = pd.DataFrame({'Name':init_pop_names, 'ELO':elos, 'Move_Seq':move_seqs}, columns=['Name','ELO', 'Move_Seq'])
df_init['Wins'] = 0
df_init['Losses'] = 0
...
default =0
df_stats = pd.DataFrame({"Gen":[default], "Name":['none'],"Wins":[default],"Loses":[default], "Draws":[default]},columns=["Gen","Name","Wins","Loses","Draws"])
list_df_stats.append(df_stats)
